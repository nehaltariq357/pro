...
async function main() {
  ...
  if (!pinVerified) {
    console.log(
      chalk.underline.bgRedBright(`You have reached maximum attempts.`)
    );
    return;
  }
...
async function depositBalance() {
  const { amount } = await inquirer.prompt([
    {
      ...
      validate: function (value) {
        ...
        if (isNaN(parsedvalue) || parsedvalue <= 0) {
          return chalk.redBright(
            `Please enter a valid positive amount for deposit`
          );
        }
        return true;
      },
  ...
  console.log(
    chalk.greenBright(
      `Successfully deposited ${depositAmount}. Your updated balance is ${balance}`
    )
  );
}
...
async function withdrawBalance() {
  const { amount } = await inquirer.prompt([
    {
      ...
      type: "input",
      message: chalk.bgBlueBright.bold(
        "Please enter the amount you wish to withdraw (multiples of 500 only):"
      ),
      validate: function (value) {
        const parsedvalue = parseInt(value);
        if (
  ...
  console.log(
    chalk.bgGreenBright(`Successfully withdrew ${withdrawalAmount}. Your remaining balance is ${balance}`)
  );
}
